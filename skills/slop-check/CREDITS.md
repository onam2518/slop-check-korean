...
- `references/persona.md` and `references/korean-baseline.md` derive the standard-voice defaults (종결·목적 연결어미·문장 길이·표기) and craft targets (어휘 다양성·리듬·구체성) from two 국립국어원 2025 corpora: 신문 말뭉치 (NIKL_NEWSPAPER_2025_v1.0, ~860k-paragraph sample) for the register standard, and 문어 말뭉치 (NIKL_WRITTEN_2025_v1.0, 책-정보 nonfiction, ~180k-sentence sample; 문학 excluded) for craft. Only aggregate statistics are used; no corpus text is redistributed here.
